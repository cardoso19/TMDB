...
class MovieCollectionViewCell: UICollectionViewCell {
    ...
    func setContent(item: Movie) {
        ...
        if let date = item.release_date?.toDate(format: "yyyy/MM/dd") {
            labelReleaseDate.text = date.dateString(ofStyle: .short)
        } else {
            labelReleaseDate.text = "-/-/-"
        }
        if let genreID = item.genre_ids?.first, let genreName = MDTGenres.shared.getMovieGenreBy(genreID: genreID) {
